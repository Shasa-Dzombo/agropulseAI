"""
AWS SageMaker/Bedrock and Azure Computer Vision integrations - unconfigured in
this environment (AWS_SAGEMAKER_ENDPOINT is unset, AWS_BEDROCK_MODEL_ID
targets the retired Claude 2 completions API, AZURE_SUBSCRIPTION_KEY is
unset). See app/services/ai_service.py for the actual working AI path
(multi-provider: OpenAI-compatible/Anthropic/Ollama/open-weight).

This used to live in app/services/ai_service.py itself before that module
was replaced by the multi-provider one above - moved here unchanged so
app/api/cctv.py's `from app.services.ai_service import aws_ai_service` doesn't
break, without mixing this dead AWS/Azure code into the live module.
"""
import boto3
import io
